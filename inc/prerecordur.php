  ...
  { 
	 ...
	 {   
        ...
        // $to - кому отправляем 
		 ...
		 $mail_to3= 'R924@NCA.BY';
		 $mail_to4= 'R1747@NCA.BY';
        ...
        {
	        $soob='                        <b><font color="#F00" size="3pt" >Запрос на запись не отправлен. В поле "Фамилия, Имя, Отчество" допускается вводить только буквы и знаки препинания.</font></b>';
	   ...
	   {	  
	        ...
	        $subject='Предварительная запись с сайта. Юр. лица';
	        $message=' Фамилия Имя Отчество:'.$_POST['fioman'].' Адрес недвижимости, по которому обращаются:'.$_POST['address'].', e-mail: '.$_POST['email'].', Телефон: '.$_POST['tel'].', Дата, на которую хотят записаться: '.$_POST['daterec'].', Время, которое хотят записаться: '.$_POST['timerec'].', Причина обращения:'.$_POST['question'];
			...
			$sended3 = smtpmail($mail_to3, $subject, $message, $headers);
			$sended4 = smtpmail($mail_to4, $subject, $message, $headers);
	        if ((!$sended)||(!$sended1)||(!$sended2)) echo '<b><font color="#F00" size="3pt" >Ошибка отправки. Приносим свои извинения. Вы можете задать свой вопрос, отправив его на электронную почту агентства d200@nca.by. </font></b>'.$mail_to;
       ...
       $my->in_question=$_POST['question'];//$question;
       $my->fioman=$_POST['fioman'];//$fioman;
       $my->address=$_POST['address'];//$address;
       $my->email= $_POST['email'];//$email;
       $my->tel= $_POST['tel'];//$tel;
       $my->daterec=$_POST['daterec'];//$daterec;
       $my->timerec=$_POST['timerec'];//$timerec;
...
?>
<div class="TD1"><!--<b>По вопросам государственной регистрации юридических лиц и индивидуальнызх предпринимателей<br /> Ответственное лицо: начальник сектора по работе с юридическими лицами управления &laquo;Регистр&raquo; Игнатенко Ольга Владимировна (каб. 402)<br> телефон: +372 (212) 60 81 70</b>--><br />
Предварительная запись осуществляется не ранее чем за сутки от желаемой даты и времени приема.</div><br />
         ...
         <form action="<? $PHP_SELF; ?>" method="post" class="form_"> 
			 <div class="align_center" > 
              <input type="hidden" name="tt" value="Y">
			   <b>Наименование организации (наименование организации вводится без кавычек) или Фамилия, Имя, Отчество представителя:</b><br/>
